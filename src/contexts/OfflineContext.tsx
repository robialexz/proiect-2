...
export function OfflineProvider({ children }: { children: React.ReactNode }) {
  ...
  const handleOnline = () => {
    ...
    toast({
      ...
      description: t('offline.onlineDescription', 'Your device is now connected to the internet.'),
      variant: 'default',
    });
  ...
  const handleOffline = () => {
    ...
    toast({
      ...
      description: t('offline.offlineDescription', 'Your device is not connected to the internet.'),
      variant: 'destructive',
    });
  };
  ...
  const toggleOfflineMode = () => {
    ...
    toast({
      ...
      description: !isOfflineMode
        ? t('offline.offlineModeEnabledDescription', 'You can now work offline. Changes will be synced when you reconnect.')
        : t('offline.offlineModeDisabledDescription', 'Your changes will be synced immediately.'),
      variant: 'default',
    });
  };
  ...
  const syncData = async () => {
    if (!isOnline) {
      toast({
        ...
        description: t('offline.cannotSyncDescription', 'You are offline. Please connect to the internet to sync your changes.'),
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        toast({
          ...
          description: t('offline.syncSuccessDescription', 'Successfully synced {{count}} changes.', { count: result.syncedCount }),
          variant: 'default',
        });
      } else {
        toast({
          title: t('offline.syncPartialTitle', 'Sync Partially Complete'),
          description: t('offline.syncPartialDescription', 'Synced {{syncedCount}} changes, but {{failedCount}} failed.', {
      ...
      toast({
        ...
        description: t('offline.syncErrorDescription', 'An error occurred while syncing your changes.'),
        variant: 'destructive',
      });
    }
  };
