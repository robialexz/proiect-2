...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Reîmprospătăm sesiunea la fiecare 20 de minute pentru a preveni expirarea
    const refreshInterval = setInterval(async () => {
      console.log("[AuthContext] Auto-refreshing session");
      try {
        // Verificăm dacă sesiunea este aproape de expirare
        ...
        if (localSession) {
          try {
            ...
            // Reîmprospătăm dacă mai sunt mai puțin de 6 ore până la expirare
            if (parsedSession.expiresAt && (parsedSession.expiresAt - Date.now()) > 6 * 3600 * 1000) {
              console.log("[AuthContext] Session still valid, skipping refresh");
              shouldRefresh = false;
            }
          } catch (parseError) {
            console.error("[AuthContext] Error parsing session for refresh check:", parseError);
          }
        }
        ...
        if (shouldRefresh) {
          ...
          if (error) {
            ...
            setTimeout(async () => {
              console.log("[AuthContext] Retrying session refresh");
              const retryResult = await supabase.auth.refreshSession();
              if (retryResult.error) {
                console.error("[AuthContext] Retry failed:", retryResult.error);
              } else if (retryResult.data.session) {
                console.log("[AuthContext] Session refresh retry successful");
            ...
            }, 5000); // Încercăm din nou după 5 secunde
