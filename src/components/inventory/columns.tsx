...
interface ColumnsProps {
  ...
  setMaterialToRequestSuplimentar: React.Dispatch<React.SetStateAction<Material | null>>;
  setMaterialToConfirmSuplimentar: React.Dispatch<React.SetStateAction<Material | null>>;
  ...
  const columns: ColumnDef<Material>[] = [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div
            ...
            title={t('inventory.actions.adjustTooltip', 'Click to adjust supplementary quantity') || ''}
          >
            {material.name}
          </div>
        )
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className={`text-right font-medium ${suplimentar > 0 ? 'text-yellow-400' : ''}`}>
            {suplimentar}
          </div>
        )
      },
    ...
    {
      id: "actions",
      header: () => <div className="text-right">{t('common.actions', 'Actions')}</div>, // Translate header
      cell: ({ row }) => {
        const material = row.original;
        return (
          <div className="text-right" data-col-id="actions"> {/* Ensure data-col-id is here */}
            <DropdownMenu>
              {/* Removed asChild from DropdownMenuTrigger */}
              <DropdownMenuTrigger>
                <Button variant="ghost" className="h-8 w-8 p-0">
                  <span className="sr-only">{t('inventory.actions.openMenu', 'Open menu')}</span>
                ...
                >
                 ...
                 {(material.suplimentar ?? 0) > 0 && (
                   <>
                     ...
                     >
                       {t('inventory.actions.confirmSuplimentar', 'Confirm Suplimentar')} ({material.suplimentar})
                     </DropdownMenuItem>
                   </>
                 )}
              </DropdownMenuContent>
