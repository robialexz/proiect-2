...
export class Logger {
  ...
  constructor(config: Partial<LoggerConfig> = {}) {
    ...
    if (this.config.enabled && this.config.logToServer) {
      this.flushInterval = window.setInterval(() => this.flushLogQueue(), this.config.batchInterval);
    }
  }
  ...
  private async flushLogQueue(): Promise<void> {
    if (!this.config.enabled || !this.config.logToServer || this.logQueue.length === 0) {
      return;
    }
