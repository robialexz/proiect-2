...
const AppLayout: React.FC = () => {
  ...
  // Afișăm un mesaj de bun venit DOAR la autentificare, niciodată altcândva
  useEffect(() => {
    // Verificăm dacă este o nouă autentificare
    const isNewLogin = sessionStorage.getItem("newLoginDetected");

    if (user && !loading && isNewLogin === "true") {
      // Afișăm overlay-ul de bun venit
      setShowWelcomeOverlay(true);

      // Resetăm flag-ul de nouă logare imediat
      sessionStorage.removeItem("newLoginDetected");

      // Setăm un timeout pentru a ascunde overlay-ul după 5 secunde
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-900 text-white overflow-hidden">
      {/* Overlay de bun venit - afișat doar la autentificare */}
      {showWelcomeOverlay && <WelcomeOverlay onComplete={() => setShowWelcomeOverlay(false)} />}

      {/* Componenta de stare a conexiunii */}
      <ConnectionStatus />

      {/* Structură principală cu o singură bară de scroll */}
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar - ascuns pe mobil când este închis */}
        <div
          className={`fixed inset-y-0 left-0 z-40 transform ${
            isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"
          } md:relative md:translate-x-0 transition-transform duration-300 ease-in-out overflow-hidden`}
        >
          <Sidebar />
        </div>
        ...
        <div className="flex flex-col flex-1 w-full md:w-auto overflow-hidden">
          ...
          {/* Main content - singura zonă cu overflow-auto */}
