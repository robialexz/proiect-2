  ...
  return (
    <header className="bg-slate-800 border-b border-slate-700 py-3 px-4 flex items-center justify-between">
      ...
      <div className="flex items-center space-x-2">
        ...
        {/* Dropdown for additional actions on smaller screens */}
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="bg-slate-800 border-slate-700">
            <DropdownMenuItem onClick={() => navigate("/tutorial")}>
              ...
              <span>{t("navbar.help", "Ajutor")}</span>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={handleThemeToggle}>
              {theme === "dark" ? (
                <>
                  <Sun className="mr-2 h-4 w-4" />
                  <span>{t("navbar.lightMode", "Mod luminos")}</span>
                </>
              ) : (
                <>
                  <Moon className="mr-2 h-4 w-4" />
                  <span>{t("navbar.darkMode", "Mod întunecat")}</span>
                </>
              )}
            </DropdownMenuItem>
          </DropdownMenuContent>
