...
export const useSuppliers = () => {
  ...
  const loadSuppliers = useCallback(async () => {
    ...
    try {
      // În loc să folosim funcția RPC care lipsește, vom face o interogare direct
      // Obținem furnizorii din tabelul suppliers
      ...
      // Transformăm datele pentru a adăuga detaliile care ar fi fost furnizate de funcția RPC
      const suppliersWithDetails = suppliersData.map(supplier => ({
        ...supplier,
        last_order: null, // Nu avem acces la ultima comandă fără funcția RPC
        materials_count: 0 // Nu avem acces la numărul de materiale fără funcția RPC
      }));
