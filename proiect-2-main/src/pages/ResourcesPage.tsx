...
// Folosim tipul Resource din @/types/supabase
...
const ResourcesPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadProjects = async () => {
      ...
      try {
        ...
        // Convertim datele la tipul Project complet
  ...
  const {
    ...
    "materials", // Folosim tabela materials în loc de resources
    "*",
  ...
  useEffect(() => {
    const loadResources = async () => {
      if (resourcesData) {
        // Transformăm materialele în resurse folosind serviciul
  ...
  const fetchResourcesFallback = async () => {
    try {
      setIsLoading(true);
      // Folosim serviciul supabaseService în loc de apelul direct la supabase
      const resources = await supabaseService.getResources();
      setResources(resources);
    } catch (error: any) {
      // Removed console statement
