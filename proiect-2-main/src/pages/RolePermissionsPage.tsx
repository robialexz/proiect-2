...
const RolePermissionsPage: React.FC = () => {
  ...
  const { } = useAuth(); // Removed unused userRole
  const { canAccess } = usePermissionCheck();
  const { toast } = useToast();

  // Verificăm dacă utilizatorul are acces la această pagină
  // Pentru moment, permitem accesul tuturor utilizatorilor pentru testare
        ...
        <AnimatedContainer animation="slideUp">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              >
                {t("rolePermissions.pageTitle", "Administrare roluri și permisiuni")}
              </AnimatedText>
